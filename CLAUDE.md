...
`.agents/skills/` is a generated, flattened mirror of the promoted buckets, produced by `scripts/sync-agents-skills.sh`. Harnesses that only scan `.agents/skills/<skill-name>/SKILL.md` (Devin Cloud among them, which indexes a repo's committed contents, so symlinks are not an option) read the skills from there. Never edit files under `.agents/skills/`: change the skill under `skills/` and re-run the script, which is also the step to repeat after syncing with upstream. To refresh the mirror straight from `mattpocock/skills` without a git-level sync, run the manual `Sync skills from upstream` workflow (`.github/workflows/sync-skills.yml`), optionally passing an upstream tag to pin the version.
